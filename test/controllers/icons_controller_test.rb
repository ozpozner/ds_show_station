...
class IconsControllerTest < ActionController::TestCase
  ...
  test "should create icon" do
    assert_difference('Icon.count') do
      post :create, icon: { altText: @icon.altText, height: @icon.height, url: @icon.url, width: @icon.width }
    end
  ...
  test "should update icon" do
    patch :update, id: @icon, icon: { altText: @icon.altText, height: @icon.height, url: @icon.url, width: @icon.width }
    assert_redirected_to icon_path(assigns(:icon))
  end
